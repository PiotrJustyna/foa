...
%%--------------------------------------------------------------------
content_types_provided(Req, State) -> {[{{<<"application">>, <<"json">>, []}, get_feedback}], Req, State}.

%%--------------------------------------------------------------------
content_types_accepted(Req, State) -> {[{{<<"application">>, <<"json">>, []}, create_feedback}], Req, State}.
...
create_feedback(Req0, State) ->
    ...
    case jsx:is_json(Data) of
        ...
        true ->
            ...
            Req2 = cowboy_req:set_resp_body(io_lib:format("{\"id\": \"~s\"}", [FeedbackId]), Req1),
            {true, Req2, State}
    end.
...
get_file_name(FileId) -> io_lib:format("feedback/~s.json", [FileId]).
